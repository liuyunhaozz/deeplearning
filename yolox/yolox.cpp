...
#define BBOX_CONF_THRESH 0.0001
...
void FindFiles(std::string root ,std::vector<std::string> &files){
    DIR *dir;
    struct dirent *ent;
...
static void write2csv(const std::vector<Object>& objects, std::string f, std::ofstream &file) {
    static const char* class_names[] = {
        "holothurian", "echinus", "scallop", "starfish"
    };

    int index = f.find('.');
    std::string id = f.substr(0, index);

    for (size_t i = 0; i < objects.size(); i++)
    {
        const Object& obj = objects[i];
        
        file << class_names[obj.label] << "," << id << "," << obj.prob << "," << obj.rect.x - obj.rect.width / 2 << "," << obj.rect.y - obj.rect.height / 2
            << "," << obj.rect.x + obj.rect.width / 2 << "," << obj.rect.y + obj.rect.height / 2 << "\n";
...
int main(int argc, char** argv) {
    ...
    FindFiles(argv[3], filelist);

    std::ofstream csvfile("output.csv", std::ofstream::out);
    csvfile << "name,image_id,confidence,xmin,ymin,xmax,ymax\n";
    ...
    for (auto imgname = filelist.begin(); imgname != filelist.end(); ++imgname) {
        const std::string input_image_path {argv[3] + *imgname};
        ...
        write2csv(objects, *imgname, csvfile);
